...
void EXTI0_IRQHandler(void) {
	...
	if (exti2pd[0]) {
		if (exti2pd[0]->callback) {
			exti2pd[0]->callback(exti2pd[0]->userdata);
		} else {
			io_printf("no registered callback function\n");
		}
	} else {
		io_printf("no pin in use for irq\n");
	}
	exti_regs->pr=(1<<0);
}
...
void EXTI1_IRQHandler(void) {
	...
	if (exti2pd[1]) {
		if (exti2pd[1]->callback) {
			exti2pd[1]->callback(exti2pd[1]->userdata);
		} else {
			io_printf("no registered callback function\n");
		}
	} else {
...
void EXTI2_IRQHandler(void) {
	...
	if (exti2pd[2]) {
		if (exti2pd[2]->callback) {
			exti2pd[2]->callback(exti2pd[2]->userdata);
		} else {
			io_printf("no registered callback function\n");
		}
	} else {
...
void EXTI3_IRQHandler(void) {
	...
	if (exti2pd[3]) {
		if (exti2pd[3]->callback) {
			exti2pd[3]->callback(exti2pd[3]->userdata);
		} else {
			io_printf("no registered callback function\n");
		}
	} else {
...
void EXTI4_IRQHandler(void) {
	if (exti2pd[4]) {
		if (exti2pd[4]->callback) {
			exti2pd[4]->callback(exti2pd[4]->userdata);
		} else {
			io_printf("no registered callback function\n");
		}
	} else {
...
void EXTI9_5_IRQHandler(void) {
	io_printf("exti9_5_irq\n");
	exti_regs->pr=0x03e0;
}

void EXTI15_10_IRQHandler(void) {
	io_printf("exti15_10_irq\n");
	exti_regs->pr=0xfc00;
}
...
static int assign_pin(struct pin_data *pdp, int pin) {
	...
	io_printf("lookup pin %d at bus %d\n",bpin,bus);

	if (pinmap[bus]&(1<<bpin)) {
		io_printf("pin in use %x\n", pin);
		return -1;
	}
...
static int set_flags(struct pin_data *pdp, unsigned int flags) {
	...
	if (flags&GPIO_OUTPUT) {
		...
		GPIO[pdp->bus]->ospeedr|=(1<<(pdp->bpin<<1));
		
	} else {
		io_printf("pin is input\n");
		GPIO[pdp->bus]->moder&=~(3<<(pdp->bpin<<1));

		switch(drive) {
			case GPIO_FLOAT: {
				io_printf("gpio float\n");
				GPIO[pdp->bus]->pupdr&=~(3<<(pdp->bpin<<1));
				break;
			}
			case GPIO_PULLUP: {
				io_printf("gpio pullup\n");
				GPIO[pdp->bus]->pupdr&=~(3<<(pdp->bpin<<1));
				GPIO[pdp->bus]->pupdr|=(1<<(pdp->bpin<<1));
				break;
			}
			case GPIO_PULLDOWN: {
				io_printf("gpio_pulldown\n");
				GPIO[pdp->bus]->pupdr&=~(3<<(pdp->bpin<<1));
				GPIO[pdp->bus]->pupdr|=(2<<(pdp->bpin<<1));
				break;
			}
			default: {
				io_printf(" gpio unknown drive mode %d\n",drive);
			}
		}
	}
	...
	if (flags&GPIO_IRQ) {
		...
		io_printf("enable irq\n");
		if (exti2pd[pdp->bpin]) {
			io_printf("interrupt line cannot not be assigned, in use\n");
			return -1;
		}	
...
static int gpio_open(void *instance, DRV_CBH callback, void *userdata) {
