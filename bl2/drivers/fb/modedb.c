...
static int fb_try_mode(struct fb_var_screeninfo *var, struct fb_info *info,
    ...
    sys_printf("Trying mode %s %dx%d-%d@%d\n", mode->name ? mode->name : "noname",
                 ...
                 unsigned int default_bpp)
{
    ...
    if (mode_option) {
        ...
        if (cvt) {
            ...
            int ret;
            sys_printf("CVT mode %dx%d@%dHz%s%s%s\n", xres, yres,
            ...
            if (!ret && !fb_try_mode(var, info, &cvt_mode, bpp)) {
                sys_printf("modedb CVT: CVT mode ok\n");
                return 1;
            }

            sys_printf("CVT mode invalid, getting mode from database\n");
        }

        sys_printf("Trying specified video mode%s %ix%i\n",
 ...
 */
        ...
        diff = xres + yres;
        best = -1;
        sys_printf("Trying best-fit modes\n");
        for (i = 0; i < dbsize; i++) {
            if (xres <= db[i].xres && yres <= db[i].yres) {
                sys_printf("Trying %ix%i\n", db[i].xres, db[i].yres);
    ...
    sys_printf("Trying default video mode\n");
    if (!fb_try_mode(var, info, default_mode, default_bpp))
        return 3;
    sys_printf("Trying all modes\n");
    for (i = 0; i < dbsize; i++)
        if (!fb_try_mode(var, info, &db[i], default_bpp))
            return 4;

    sys_printf("No valid mode found\n");
    return 0;
}
...
void fb_var_to_videomode(struct fb_videomode *mode,
        ...
        unsigned int pixclock, hfreq, htotal, vtotal;
...
*fb_find_best_mode(const struct fb_var_screeninfo *var,
	...
	LIST_FOREACH(modelist, head, list) {
		unsigned int d;
		mode = &modelist->mode;
...
int fb_add_videomode(const struct fb_videomode *mode,
	...
	if (!found) {
		modelist = malloc(sizeof(struct fb_modelist));

		if (!modelist)
			return -ENOMEM;
		modelist->mode = *mode;
		LIST_INSERT_HEAD(head,modelist, list);
        }
        return 0;
}
...
void fb_delete_videomode(const struct fb_videomode *mode,
				struct modelist *head) {
	struct fb_modelist *pos, *n;
        struct fb_modelist *modelist;
        struct fb_videomode *m;

	LIST_FOREACH_SAFE(modelist, head, list, n) {
		m = &modelist->mode;
		if (fb_mode_is_equal(m, mode)) {
			LIST_REMOVE(modelist,list);
			free(modelist);
		}
	}
}
 ...
 * @head: struct list_head of modelist
 */
void fb_destroy_modelist(struct modelist *head) {
        struct fb_modelist *pos, *n;

        LIST_FOREACH_SAFE(pos, head, list, n) {
