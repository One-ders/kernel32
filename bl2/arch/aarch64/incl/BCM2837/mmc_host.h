...
#define MMC_SET_DRIVER_TYPE_D	3
	...
	int enhanced_strobe;			/* hs400es selection */
};

struct mmc_clk_phase {
	int valid;
	unsigned short in_deg;
	unsigned short out_deg;
};
...
struct mmc_host_ops {
	...
	int	(*execute_tuning)(struct mmc_host *host, unsigned int opcode);
...
struct mmc_cqe_ops {
	 ...
	 * recovery is needed.
	 */
	int	(*cqe_timeout)(struct mmc_host *host, struct mmc_request *mrq,
			       int *recovery_needed);
	/*
...
//	struct wakeup_source	*ws;		/* Enable consume of uevents */
	unsigned int 		max_current_330;
	unsigned int 		max_current_300;
	unsigned int 		max_current_180;
...
//	mmc_pm_flag_t		pm_flags;	/* requested pm features */
	...
	int			regulator_enabled; /* regulator state */
#endif
	...
	unsigned int		dsr;	/* optional driver stage (DSR) value */
	...
	unsigned int		err_stats[MMC_ERR_MAX];
 ...
 */
static inline int sdio_irq_claimed(struct mmc_host *host)
{
...
static inline int mmc_can_retune(struct mmc_host *host)
{
	return host->can_retune == 1;
}

static inline int mmc_doing_retune(struct mmc_host *host)
{
	return host->doing_retune == 1;
}

static inline int mmc_doing_tune(struct mmc_host *host)
{
