...
#define	MSC_BASE(n)	(0xB0021000+(n*0x1000))
...
/* Register Offset */
#define OFF_RBR		(0x00)	/* R  8b H'xx */
#define OFF_THR		(0x00)	/* W  8b H'xx */
#define OFF_DLLR	(0x00)	/* RW 8b H'00 */
#define OFF_DLHR	(0x04)	/* RW 8b H'00 */
#define OFF_IER		(0x04)	/* RW 8b H'00 */
#define OFF_IIR		(0x08)	/* R  8b H'01 */
...
#define OFF_ISR		(0x20)	/* RW 8b H'00, UART0 */
#define OFF_UMR		(0x24)	/* RW 8b H'00, UART M Register */
#define OFF_UACR	(0x28)	/* RW 8b H'00, UART Add Cycle Register */

/* Register Address */
#define UART0_RBR	(UART0_BASE + OFF_RBR)
#define UART0_THR	(UART0_BASE + OFF_THR)
#define UART0_DLLR	(UART0_BASE + OFF_DLLR)
#define UART0_DLHR	(UART0_BASE + OFF_DLHR)
#define UART0_IER	(UART0_BASE + OFF_IER)
#define UART0_IIR	(UART0_BASE + OFF_IIR)
...
#define UART_IER_RLSIE	(1 << 2)	/* 0: receive line status interrupt disable */
#define UART_IER_MSIE	(1 << 3)	/* 0: modem status interrupt disable */
#define UART_IER_RTOIE	(1 << 4)	/* 0: receive timeout interrupt disable */

/*
 * Define macros for UART_IIR
 * UART Interrupt Status Register
 */
#define UART_IIR_IP	(1 << 0)	/* 0: interrupt is pending  1: no interrupt */
#define UART_IIR_IID	(7 << 1)	/* Source of Interrupt */
#define UART_IIR_IID_MSI		(0 << 1)	/* Modem status interrupt */
#define UART_IIR_IID_THRI	(1 << 1)	/* Transmitter holding register empty */
#define UART_IIR_IID_RDI		(2 << 1)	/* Receiver data interrupt */
#define UART_IIR_IID_RLSI	(3 << 1)	/* Receiver line status interrupt */
#define UART_IIR_FFMS	(3 << 6)	/* FIFO mode select, set when UART_FCR.FE is set to 1 */
#define UART_IIR_FFMS_NO_FIFO	(0 << 6)
#define UART_IIR_FFMS_FIFO_MODE	(3 << 6)
 ...
 */
#define UART_LCR_WLS 	(3 << 0)	/* word length */
#define UART_LCR_WLS_5	(0 << 0)
#define UART_LCR_WLS_6	(1 << 0)
#define UART_LCR_WLS_7	(2 << 0)
#define UART_LCR_WLS_8	(3 << 0)
#define UART_LCR_SBLS	(1 << 2)	/* 0: 1 stop bit when word length is 5,6,7,8
					   1: 1.5 stop bits when 5; 2 stop bits when 6,7,8 */
#define UART_LCR_SBLS_1	(0 << 2)	/* 0: 1 stop bit when word length is 5,6,7,8
					   1: 1.5 stop bits when 5; 2 stop bits when 6,7,8 */
#define UART_LCR_SBLS_2	(1 << 2)	/* 0: 1 stop bit when word length is 5,6,7,8
					   1: 1.5 stop bits when 5; 2 stop bits when 6,7,8 */

#define UART_LCR_PARE	(1 << 3)	/* 0: parity disable */
#define UART_LCR_PARM	(1 << 4)	/* 0: even parity  1: odd parity */
#define UART_LCR_STPAR	(1 << 5)	/* 0: sticky parity disable */
#define UART_LCR_SBK	(1 << 6)	/* write 0 normal, write 1 send break */
...
#define UART_LSR_OVER	(1 << 1)	/* 0: no overrun error */
#define UART_LSR_PARER	(1 << 2)	/* 0: no parity error */
#define UART_LSR_FMER	(1 << 3)	/* 0; no framing error */
#define UART_LSR_BI	(1 << 4)	/* 0: no break detected  1: receive a break signal */
#define UART_LSR_TDRQ	(1 << 5)	/* 1: transmit FIFO half "empty" */
#define UART_LSR_TEMP	(1 << 6)	/* 1: transmit FIFO and shift registers empty */
#define UART_LSR_FIFOE	(1 << 7)	/* 0: no receive error  1: receive error in FIFO mode */
...
#define UART_MCR_FCM	(1 << 6)	/* 0: Flow ctrl sw, 1: hw */
 ...
 */
#define UART_MSR_CCTS 	(1 << 0)	/* 1: Change CTS */
#define UART_MSR_CTS 	(1 << 4)	/* CTS Stat 0: cts hi, 1: cts lo */

/*
 * Define macros for ISR
 * Slow IrDA Infrared Selection Register
 */
#define ISR_XMITIR	(1 << 0)	/* 0: transmitter is in UART mode  1: IrDA mode */
#define ISR_RCVEIR	(1 << 1)	/* 0: receiver is in UART mode  1: IrDA mode */
#define ISR_XMODE	(1 << 2)	/* 0: transmit 0 pulse width is 3/16 of bit length
					   1: 0 pulse width is 1.6us for 115.2Kbps */
#define ISR_TDPL	(1 << 3)	/* 0: encoder generates a positive pulse for 0 */
#define ISR_RDPL	(1 << 4)	/* 0: decoder interprets positive pulse as 0 */
...
/*************************************************************************
 * MSC:  n=0 MSC0, n=1 MSC1
 *************************************************************************/
#define	MSC_STRPCL(n)		(MSC_BASE(n) + 0x000)
#define	MSC_STAT(n)		(MSC_BASE(n) + 0x004)
#define	MSC_CLKRT(n)		(MSC_BASE(n) + 0x008)
#define	MSC_CMDAT(n)		(MSC_BASE(n) + 0x00C)
#define	MSC_RESTO(n)		(MSC_BASE(n) + 0x010)
#define	MSC_RDTO(n)		(MSC_BASE(n) + 0x014)
#define	MSC_BLKLEN(n)		(MSC_BASE(n) + 0x018)
#define	MSC_NOB(n)		(MSC_BASE(n) + 0x01C)
#define	MSC_SNOB(n)		(MSC_BASE(n) + 0x020)
#define	MSC_IMASK(n)		(MSC_BASE(n) + 0x024)
#define	MSC_IREG(n)		(MSC_BASE(n) + 0x028)
#define	MSC_CMD(n)		(MSC_BASE(n) + 0x02C)
#define	MSC_ARG(n)		(MSC_BASE(n) + 0x030)
#define	MSC_RES(n)		(MSC_BASE(n) + 0x034)
#define	MSC_RXFIFO(n)		(MSC_BASE(n) + 0x038)
#define	MSC_TXFIFO(n)		(MSC_BASE(n) + 0x03C)
...
#define	REG_MSC_IMASK(n)	REG16(MSC_IMASK(n))
